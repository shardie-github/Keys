...
function KeyDetailContent({ slug }: { slug: string }) {
  ...
  const fetchPreview = useCallback(
    async (isPublic: boolean, token?: string) => {
      try {
        const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001';
  ...
  const fetchKey = useCallback(async () => {
    try {
      ...
      if (demoKey) {
        ...
        const relatedDemoKeys = DEMO_KEYS.filter((k) => k.id !== demoKey.id)
          .slice(0, 3)
          .map((k) => ({
            id: k.id,
            slug: k.slug,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
          ...
          >
            <div className="border rounded-lg p-4 sm:p-6 sticky top-4 dark:bg-slate-800 dark:border-slate-700">
                    ...
                    {isAuthenticated ? (
                      ...
                      <>
                        ...
                        <motion.button
                          ...
                          whileTap={{ scale: 0.98 }}
                          onClick={() =>
                            router.push('/signin?returnUrl=' + encodeURIComponent(`/marketplace/${slug}`))
                          }
              ...
              {key.hasAccess && <ValueReminder keyTitle={key.title} version={selectedVersion || key.version} />}
...
export function KeyDetailClient({ slug }: { slug: string }) {
  return (
    <Suspense
      fallback={
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          <div className="text-center">
            <div className="inline-block w-8 h-8 border-4 border-blue-600 border-t-transparent rounded-full mb-4 animate-spin" />
            <p className="text-gray-600 dark:text-gray-400">Loading key...</p>
          </div>
        </div>
      }
    >
      <KeyDetailContent slug={slug} />
    </Suspense>
  );
}
